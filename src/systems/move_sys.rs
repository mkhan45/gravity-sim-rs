...
pub struct PreviewMoveSys;

impl<'a> System<'a> for PreviewMoveSys{
    type SystemData = (WriteStorage<'a, Pos>, WriteStorage<'a, Movement>, Read<'a, TimeStep>, ReadStorage<'a, PreviewFlag>, Read<'a, SimSpeed>, Read<'a, PredictionSpeed>);

    fn run(&mut self, (mut pos, mut movement, time_step, flags, sim_speed, prediction_speed): Self::SystemData){
        if prediction_speed.0 > sim_speed.0{
            for _i in sim_speed.0..prediction_speed.0 {
                (&mut pos, &mut movement, &flags).par_join().for_each(|(pos, movements, _flag)|{
                    let d_t = time_step.0;
                    movements.vel.0 += (movements.accel.0 + movements.past_accel.0)/2.0 * d_t;
                    movements.vel.1 += (movements.accel.1 + movements.past_accel.1)/2.0 * d_t;
                    pos.x += movements.vel.0 * d_t + movements.accel.0/2.0 * d_t.powi(2);
